...
public class YahooFinance {
	...
	/**
	 * Sends a basic quotes request to Yahoo Finance. This will return a
	 * {@link Stock} object with its
	 * {@link yahoofinance.quotes.stock.StockQuote},
	 * {@link yahoofinance.quotes.stock.StockStats} and
	 * {@link yahoofinance.quotes.stock.StockDividend} member fields filled in
	 * with the available data. Returns null if the data can't be retrieved from
	 * Yahoo Finance.
	 * 
	 * @param symbol
	...
	/**
	 * Same as the <code>get(String)</code> method, but with the option to
	 * include historical stock quote data. Including historical data will cause
	 * the {@link Stock} object's member field
	 * {@link yahoofinance.histquotes.HistoricalQuote} to be filled in with the
	 * default past year term at monthly intervals. Returns null if the data
	 ...
	 *            indicates if the historical quotes should be included.
	 * @return a {@link Stock} object containing the requested information
	 * @throws java.io.IOException
	 *             when there's a connection problem
	 */
	...
	/**
	 * Sends a basic quotes request to Yahoo Finance. This will return a
	 * {@link Map} object that links the symbols to their respective
	 * {@link Stock} objects. The Stock objects have their
	 * {@link yahoofinance.quotes.stock.StockQuote},
	 * {@link yahoofinance.quotes.stock.StockStats} and
	 * {@link yahoofinance.quotes.stock.StockDividend} member fields filled in
	 ...
	 * The returned Map only includes the Stocks that could successfully be
	 * retrieved from Yahoo Finance.
	 * 
	 * @param symbols
	 *            the symbols of the stocks for which you want to retrieve
	 *            information
	 ...
	 * Same as the <code>get(String[])</code> method, but with the option to
	 * include historical stock quote data. Including historical data will cause
	 * the {@link Stock} objects their member field
	 * {@link yahoofinance.histquotes.HistoricalQuote} to be filled in with the
	 * default past year term at monthly intervals.
	 * <p>
	 * The latest quotes will be retrieved in a single request to Yahoo Finance.
	 * For the historical quotes (if includeHistorical), a separate request will
	 * be sent for each requested stock. The returned Map only includes the
	 * Stocks that could successfully be retrieved from Yahoo Finance.
	 * 
	 * @param symbols
	 *            the symbols of the stocks for which you want to retrieve
	 *            information
	 ...
	 * from the past year, at the specified interval. (DAILY, WEEKLY, MONTHLY)
	 * <p>
	 * The latest quotes will be retrieved in a single request to Yahoo Finance.
	 * For the historical quotes, a separate request will be sent for each
	 * requested stock. The returned Map only includes the Stocks that could
	...
	/**
	 * Sends a request for multiple stocks with the historical quotes included
	 * starting from the specified {@link Calendar} date until today, at the
	...
	/**
	 * Sends a request for multiple stocks with the historical quotes included
	 * starting from the specified {@link Calendar} date until the specified
	 * Calendar date (to) at the default interval (monthly).
	 * <p>
	 * The latest quotes will be retrieved in a single request to Yahoo Finance.
	 * For the historical quotes, a separate request will be sent for each
	 * requested stock. The returned Map only includes the Stocks that could
	 ...
	 * Sends a request for multiple stocks with the historical quotes included
	 * starting from the specified {@link Calendar} date until the specified
	 * Calendar date (to) at the specified interval.
	 * <p>
	 * The latest quotes will be retrieved in a single request to Yahoo Finance.
	 * For the historical quotes, a separate request will be sent for each
	 * requested stock. The returned Map only includes the Stocks that could
	...
	/**
	 * Sends a single request to Yahoo Finance to retrieve a quote for all the
	 * requested FX symbols. See <code>getFx(String)</code> for more information
	 * on the accepted FX symbols.
	 * 
	 * @param symbols
	 *            an array of FX symbols
	...
	private static Map<String, Stock> fetchHistoricalQuotes(Map<String, Stock> stocks, Calendar from, Calendar to,
